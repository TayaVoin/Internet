...
- Защита:
-- Запустить несколько реплик (экземпляров) Gateway. Обычно их запускают на разных физических серверах.
-- Балансировщик нагрузки перед Gateway. Перед группой Gateway ставится еще один балансировщик, который распределяет трафик между живыми экземплярами. Если один Gateway умирает, балансировщик перестает отправлять на него трафик.
-- Использовать инструменты оркестрации, типа Kubernetes, который сам следит за состоянием подов (контейнеров с гейтом) и перезапускает упавшие, а также распределяет между ними нагрузку через Service.
